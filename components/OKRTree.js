...
function OKRTree({ teamId, departmentId, statusFilter }) {
  ...
  const [formType, setFormType] = useState('root'); // 'root' or 'sub'
  const [filter, setFilter] = useState({
    team: teamId || '',
    department: departmentId || '',
    status: statusFilter || ''
  });
  // Add state for users and departments
  ...
  useEffect(() => {
    const fetchUsersAndDepartments = async () => {
      ...
      try {
        ...
        console.log('Departments data loaded successfully:', departmentsData);
  ...
  useEffect(() => {
    if (okrsList.length > 0) {
      const { nodes: treeNodes, edges: treeEdges } = buildTreeStructure(okrsList);
